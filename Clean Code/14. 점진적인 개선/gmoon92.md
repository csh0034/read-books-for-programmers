...
**`명령행 인수`** 와 **`구문`** 을 분석하기 위해 직접 구현한 **`Args`** 유틸 클래스 사례를 통해, 일반적으로 리펙토링할 시 발생되는 문제점과 해결 방법에 대해 소개한다.
...
1. 리펙토링할 클래스의 모든 시나리오에 대해 테스트 케이스를 작성한다.
   - 예외 테스트
   - 기능 테스트
   - 인수 테스트
2. 자잘한 변경을 시도한다.
...
큰 목표 하나를 이루기 위해 자잘한 단계를 수없이 거친다. 각 단계를 거쳐야 다음 단계가 가능하다. 물론 리펙토링하기 전에 작성된 모든 테스트는 통과되어야 한다. 
